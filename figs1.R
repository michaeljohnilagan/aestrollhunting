# load table
dat_o = read.csv("exper1result.csv")
...
dat = with(new.env(),{
	...
	c("accuracy_lca","f1_lca"))
	mv = subset(dat_o,method=="mv")
	names(mv) = replace(names(mv),names(mv)%in%c("accuracy","weighted_f1"),
	c("accuracy_mv","f1_mv"))
	# put together
	cbind(lca,with(mv,data.frame(accuracy_mv,f1_mv)))
})

# replace undefined with 0
dat$f1_lca = (function(x) {ifelse(is.na(x),0,x)})(dat$f1_lca)
dat$f1_mv = (function(x) {ifelse(is.na(x),0,x)})(dat$f1_mv)
...
ggplot2::ggplot(dat,ggplot2::aes(x=accuracy_mv,y=accuracy_lca))+
...
ggplot2::xlab("test set accuracy, MV")+
ggplot2::ylab("test set accuracy, LCA+SM")
...
ggplot2::aes(x=accuracy_mv,y=accuracy_lca))+
ggplot2::geom_point(alpha=0.5)+
ggplot2::geom_abline(slope=1,intercept=0)+
ggplot2::geom_hline(yintercept=0.9,lty=3)+
ggplot2::geom_vline(xintercept=0.9,lty=3)+
